...
const Login = () => {
  ...
  return (
    <div>
      ...
      <form className="absolute py-12 my-36 mx-auto right-0 left-0 w-3/12 p-12 bg-black text-white rounded-lg bg-opacity-80">
      ...
      <button className="p-4 my-6 bg-red-700 w-full rounded-lg">{isSignInForm? "Sign In" : "Sign Up"}</button>
        <p className="py-4 cursor-pointer" onClick={toggleSignInForm}>{isSignInForm? "New to Netflix? Sign In Now" : "Already Registered Member"}</p>
      </form>
    </div>
  );
};
